import modules._requests as requests
from modules._bs4 import BeautifulSoup
import re
import os
...
class Client():
    ...
    def login(self, username: str, password: str, remember: str):
        ...
        """
        self.login = session.post("https://www.animefenix.com/user/login",data={'username': username, 'password': password, 'remember': remember})
        return self.login
    ...
    def whitelist(self,series:list):
        """[summary]

        Args:
            series (list): [A list of animes name]

        Returns:
            [str]: [The file was created successfully]
        """
        try:
            with open(".whitelist.txt","a") as self.file:
                for self.serie in series:
                    self.file.write(self.serie+"\n")
                a = True
            if a == True:return "Done"
        except (IndexError,FileNotFoundError) as e: return e
    ...
    def open_new_incognito(self,navegator:str,url:str):
        """[summary]

        Args:
            navegator (str): [Navegator name, example: google-chrome]
            url (str): [Url to open in an incognito tab]
        """
        os.system(f"{navegator} --incognito %s {url}")
